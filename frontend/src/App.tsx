...
function App() {
  ...
  const requestPasswordReset = async (email: string) => {
    try {
      const response = await api('/auth/password-reset/', 'POST', { email })
      setNotice(response.detail)
    } catch (error) { setNotice(error instanceof Error ? error.message : 'Unable to request a password reset.') }
  }
  const confirmPasswordReset = async (password: string) => {
    if (!resetLink) return
    try {
      const response = await api('/auth/password-reset/confirm/', 'POST', { ...resetLink, password })
      window.history.replaceState({}, '', window.location.pathname)
      setResetLink(null); setSignUp(false); setNotice(response.detail)
    } catch (error) { setNotice(error instanceof Error ? error.message : 'Unable to reset your password.') }
  }
  ...
  if (!signedIn) return <Auth signUp={signUp} setSignUp={setSignUp} onSubmit={authenticateUser} onRequestPasswordReset={requestPasswordReset} onConfirmPasswordReset={confirmPasswordReset} resetLink={resetLink} notice={notice} />
...
function Auth({ signUp, setSignUp, onSubmit, onRequestPasswordReset, onConfirmPasswordReset, resetLink, notice }: { signUp: boolean; setSignUp: (value: boolean) => void; onSubmit: (name: string, email: string, password: string) => void; onRequestPasswordReset: (email: string) => void; onConfirmPasswordReset: (password: string) => void; resetLink: { uid: string; token: string } | null; notice: string }) {
  const [resetRequested, setResetRequested] = useState(false)
  const isResetting = Boolean(resetLink)
  const requestReset = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); onRequestPasswordReset(String(new FormData(event.currentTarget).get('email') || '')) }
  const confirmReset = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); const password = String(data.get('password') || ''); if (password !== String(data.get('passwordConfirmation') || '')) return; onConfirmPasswordReset(password) }
  return <main className="auth"><section className="auth-copy"><div className="auth-brand"><b>G</b> GoGo<span>GiftList</span></div><p className="eyebrow">Give better, together</p><h1>Thoughtful gifts.<br /><em>Zero guesswork.</em></h1><p>Bring the people who love someone together around a gift list that feels personal, organized, and joyful.</p><blockquote>“Finally, a way to make group gifting feel easy.”<small>— A very relieved gift planner</small></blockquote></section><section className="auth-form"><div>{isResetting ? <><p className="eyebrow">Account recovery</p><h2>Choose a new password</h2><p className="muted">Use a secure password with at least eight characters.</p>{notice && <p className="auth-error">{notice}</p>}<form className="form" onSubmit={confirmReset}><Label label="New password" name="password" type="password" required /><Label label="Confirm new password" name="passwordConfirmation" type="password" required /><button className="primary">Reset password <span>→</span></button></form></> : resetRequested ? <><p className="eyebrow">Account recovery</p><h2>Reset your password</h2><p className="muted">Enter your email and we’ll send a reset link if an account exists.</p>{notice && <p className="auth-error">{notice}</p>}<form className="form" onSubmit={requestReset}><Label label="Email address" name="email" type="email" placeholder="you@example.com" required /><button className="primary">Email reset link <span>→</span></button></form><p className="switch"><button onClick={() => setResetRequested(false)}>Back to sign in</button></p></> : <><p className="eyebrow">Welcome to GoGoGiftList</p><h2>{signUp ? 'Create your account' : 'Welcome back'}</h2><p className="muted">{signUp ? 'Start your first shared gift list in a few moments.' : 'Sign in to your gift lists.'}</p>{notice && <p className="auth-error">{notice}</p>}<form className="form" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); onSubmit(String(data.get('name') || ''), String(data.get('email') || ''), String(data.get('password') || '')) }}>{signUp && <Label label="Your name" name="name" placeholder="Jamie Miller" required />}<Label label="Email address" name="email" type="email" placeholder="you@example.com" required /><Label label="Password" name="password" type="password" placeholder="••••••••" required /><button className="primary">{signUp ? 'Create account' : 'Sign in'} <span>→</span></button></form>{!signUp && <p className="switch"><button onClick={() => setResetRequested(true)}>Forgot your password?</button></p>}<p className="switch">{signUp ? 'Already have an account?' : 'New to GoGoGiftList?'} <button onClick={() => setSignUp(!signUp)}>{signUp ? 'Sign in' : 'Create one'}</button></p></>}</div></section></main>
}
